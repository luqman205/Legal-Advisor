...
const LEGAL_DOCUMENTS_CORPUS = {
  ...
  "Criminal Laws": [
    ...
    },
    {
      ...
      keywords: ["self-defense", "private defense", "defend", "body", "property", "section 96", "section 100", "grievous hurt", "attacked", "justification"]
    },
    {
...
const names = ["Ahmad", "Fatima", "Zainab", "Ali", "Ayesha", "Muhammad", "Bilal", "Sana", "Usman", "Hamza"];
const cities = ["Lahore", "Karachi", "Islamabad", "Rawalpindi", "Faisalabad", "Multan", "Peshawar", "Quetta"];
const relationships = ["husband", "neighbor", "landlord", "employer", "business partner"];
const amounts = ["50,000", "100,000", "500,000", "1,000,000", "200,000"];
...
function generateDataset() {
  ...
  for (const category of CATEGORIES) {
    let topics = LEGAL_DOCUMENTS_CORPUS[category] || [];
    if (topics.length === 0) {
      topics = [{
        title: `General Statutory Codex of ${category}`,
        statute: `General Statutes regarding ${category}`,
        source: `Pakistan Statutory Laws (${category})`,
        text: `This is the official legal document regulating ${category} in the territory of Pakistan.`,
        q_templates: [`What is the legal framework regarding ${category} in Pakistan?`],
        keywords: [category.toLowerCase(), "statutory", "legal", "court"]
      }];
    }

    const recordsForThisCat = (category === "Family Laws") ? 1000 : ((((category === "Criminal Laws") || (category === "Civil Laws") || (category === "Property Laws") || (category === "Labour Laws") || (category === "Tax Laws") || (category === "Consumer Protection Laws")) ? 3000 : 150));
    let catCount = 0;
    while (catCount < recordsForThisCat) {
      const topic = topics[catCount % topics.length];
      ...
      const chosenQTemplate = topic.q_templates[Math.floor(Math.random() * topic.q_templates.length)];
      const chosenQ = chosenQTemplate.replace("{amount}", amount);

      const questionStyles = [
        `Case Ref ${String(count).padStart(5, '0')}: My name is ${name} from ${city}. ${chosenQ}`,
        `Case Ref ${String(count).padStart(5, '0')}: In ${city}, I have a dispute with my ${relation}. ${chosenQ}`,
        `Case Ref ${String(count).padStart(5, '0')}: Regarding ${topic.title} under ${topic.source || 'Pakistani Law'}: ${chosenQ}`
      ];
      const finalQ = questionStyles[Math.floor(Math.random() * questionStyles.length)];
      const finalA = topic.text.trim();

      const record = {
        id: `L_QA_${String(count).padStart(6, '0')}`,
        category: category,
        question: finalQ,
        answer: finalA,
        source: topic.source || "Constitution and Statutes of Pakistan",
        keywords: (topic.keywords || [category.toLowerCase()]).concat([city.toLowerCase(), relation]),
        difficulty: difficulties[Math.floor(Math.random() * difficulties.length)],
        language: "English",
        title: topic.title,
        statute: topic.statute || "Statutory Provisions of Pakistan"
      };
...
const targetDataset = generateDataset();

// Deduplicate questions
const seenQs = new Set();
const dedupedDataset = [];
for (const r of targetDataset) {
  if (!seenQs.has(r.question)) {
    seenQs.add(r.question);
    dedupedDataset.push(r);
  }
}

// Save directly to the Next.js database location
const workspaceJsonPath = path.join(__dirname, 'legal_ai', 'dataset', 'legal_dataset.json');
const workspaceCsvPath = path.join(__dirname, 'legal_ai', 'dataset', 'legal_dataset.csv');

// Create parent directory if it doesn't exist
fs.mkdirSync(path.dirname(workspaceJsonPath), { recursive: true });

// Write JSON
fs.writeFileSync(workspaceJsonPath, JSON.stringify(dedupedDataset, null, 4), 'utf8');
console.log(`Saved JSON dataset to ${workspaceJsonPath}`);

// Write CSV
const csvContent = [];
csvContent.push(["id", "category", "question", "answer", "source", "keywords", "difficulty", "language", "title", "statute"].join(","));
for (const r of dedupedDataset) {
  const row = [
    r.id,
    `"${r.category}"`,
...
fs.writeFileSync(workspaceCsvPath, csvContent.join("\n"), 'utf8');
console.log(`Saved CSV dataset to ${workspaceCsvPath}`);

// Also save to the legacy scratch location to remain fully compatible
const scratchJsonPath = '/Users/mac/.gemini/antigravity-ide/scratch/legal_dataset.json';
const scratchCsvPath = '/Users/mac/.gemini/antigravity-ide/scratch/legal_dataset.csv';
try {
  fs.mkdirSync(path.dirname(scratchJsonPath), { recursive: true });
  fs.writeFileSync(scratchJsonPath, JSON.stringify(dedupedDataset, null, 4), 'utf8');
  fs.writeFileSync(scratchCsvPath, csvContent.join("\n"), 'utf8');
} catch (e) {
  console.log("Could not write to scratch path, skipping.");
}
